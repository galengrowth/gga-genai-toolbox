...
type Config struct {
	...
	// Custom holds top-level tools.yaml `custom:` (billing, OAuth PRM flags, etc.).
...
func (p *ConfigParser) ParseConfig(ctx context.Context, raw []byte) (Config, error) {
	...
	config.Custom, err = extractCustomFromYAML(raw)
	if err != nil {
		return config, fmt.Errorf("error reading custom config: %w", err)
	}
