...
import (
	...
	"github.com/googleapis/genai-toolbox/internal/custom/auth/hta"
...
type ServerConfig struct {
	...
	PollInterval int
	// Custom holds user-defined settings from tools.yaml (e.g. billing, OAuth PRM) not in the core schema.
...
func UnmarshalYAMLAuthServiceConfig(ctx context.Context, name string, r map[string]any) (auth.AuthServiceConfig, error) {
	...
	case generic.AuthServiceType:
		...
		return actual, nil
	case authzero.AuthServiceType:
		actual := authzero.Config{Name: name}
		if err := dec.DecodeContext(ctx, &actual); err != nil {
			return nil, fmt.Errorf("unable to parse as %s: %w", name, err)
		}
		return actual, nil
	case hta.AuthServiceType:
